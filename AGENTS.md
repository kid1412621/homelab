...
The Kubernetes cluster may not run on this host. For code changes, use local dry checks to validate syntax and Kustomize rendering; do not run cluster-mutating commands such as `kubectl apply`, `kubectl rollout`, or `kubectl delete` unless explicitly requested.

```bash
# Render all local manifests when remote bases are available
kubectl kustomize .

# Render a specific application
kubectl kustomize <app-name>/
```
